...
class GO2_TrottoJump_Robot(BaseTask):
    def __init__(self, cfg: GO2_TrottoJump_Cfg, sim_params, physics_engine, sim_device, headless):
    ...
    def step(self, actions):
        ...
        for _ in range(self.cfg.control.decimation):
            action_delayed = self.update_cmd_action_latency_buffer()#????放在这里表示延迟了0.005秒的1到3帧月也就是最多0.015秒    jump
            ...
            self.update_obs_latency_buffer() #jump
    ...
    def post_physics_step(self):
        ...
        self.last_last_actions[:] = torch.clone(self.last_actions[:]) #jump
        self.last_actions[:] = self.actions[:]
        self.last_dof_vel[:] = self.dof_vel[:]
        self.last_root_vel[:] = self.root_states[:, 7:13]
        self.last_rigid_state[:] = self.rigid_state[:] #jump
    ...
    def reset_idx(self, env_ids):
        ...
        # reset buffers
        self.last_last_actions[env_ids] = 0. #jump
        self.actions[env_ids] = 0.
        self.last_actions[env_ids] = 0.
        self.last_rigid_state[env_ids] = 0. #jump
    ...
    def _get_gait_phase(self):
        ...
        is_jumping = (self.jump_command[:, 0] == 1)
        
        # 把跳跃的触地比例提高到 0.55
      
        stance_ratio = torch.where(is_jumping, 0.55, 0.5) 
        
        stance_mask[:, 0] = phase < stance_ratio
        stance_mask[:, 1] = phase > stance_ratio
    ...
    def _process_rigid_shape_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_friction:
            ...
            self.env_frictions[env_id] = self.friction_coeffs[env_id] #jump    
    ...
    def _post_physics_step_callback(self):
        ...
        if self.cfg.terrain.measure_heights:  #jump
            self.measured_heights = self._get_heights() #jump
    ...
    def _compute_torques(self, actions):
        ...
        """
        # pd controller (trotting and jump)
    ...
    def _reset_root_states(self, env_ids):
        ...
        if self.custom_origins:
            ...
            self.root_states[env_ids, :2] += torch_rand_float(-4., 4., (len(env_ids), 2), device=self.device) # xy position within 4m of the center
        else:
            self.root_states[env_ids] = self.base_init_state
            self.root_states[env_ids, :3] += self.env_origins[env_ids]
        # base velocities

        if self.cfg.asset.fix_base_link:  #jump
            self.root_states[env_ids, 7:13] = 0 #jump
            self.root_states[env_ids, 2] += 1.8 #jump
        env_ids_int32 = env_ids.to(dtype=torch.int32)
    ...
    def _init_buffers(self):
        ...
        self.last_last_actions = torch.zeros(self.num_envs, self.num_actions, dtype=torch.float, device=self.device, requires_grad=False) #jump
        self.last_rigid_state = torch.zeros_like(self.rigid_state) #jump
        ...
        if self.cfg.terrain.measure_heights:
            self.height_points = self._init_height_points() # jump
        self.measured_heights = 0 # jump

        # self.jump_command=torch.zeros((self.num_envs, 1), dtype=torch.float, device=self.device)
        self.jump_command = torch.ones((self.num_envs, 1), dtype=torch.float, device=self.device)
        ...
        self.rand_push_force = torch.zeros((self.num_envs, 3), dtype=torch.float32, device=self.device) #jump
        self.rand_push_torque = torch.zeros((self.num_envs, 3), dtype=torch.float32, device=self.device)  #jump
        
        self.default_dof_pos = self.default_dof_pos.unsqueeze(0)

        self.default_joint_pd_target = self.default_dof_pos.clone() #jump
    ...
    def _create_envs(self):
        ...
        print("dof_names", self.dof_names) #jump
        ...
        self.actor_handles = []
        self.envs = []
        self.env_frictions = torch.zeros(self.num_envs, 1, dtype=torch.float32, device=self.device) #jump

        self.body_mass = torch.zeros(self.num_envs, 1, dtype=torch.float32, device=self.device, requires_grad=False) #jump
    ...
    def _init_height_points(self):  #jump
    ...
    def _get_heights(self, env_ids=None):  #jump
    ...
    def _reward_trot(self):
        ...
        # 将 stance_mask 扩展为 4 条腿对应的掩码
        # 假设 stance_mask[:, 0] 是左前/右后的相位，stance_mask[:, 1] 是右前/左后的相位
        target_contact = torch.zeros_like(contact)
        target_contact[:, 0] = stance_mask[:, 0] # FL
        target_contact[:, 3] = stance_mask[:, 0] # RR
        target_contact[:, 1] = stance_mask[:, 1] # FR
        target_contact[:, 2] = stance_mask[:, 1] # RL
        
        # 计算实际接触和预期接触之间的误差
        contact_error = torch.sum(torch.abs(contact - target_contact), dim=1)
        
        # 误差越小，得分越高 (连续平滑)
        TROT_reward = torch.exp(-contact_error)
        
        return TROT_reward * (torch.norm(self.commands[:, :2], dim=1) > 0.2)
    
    def _reward_jump(self):  
        contact = (self.contact_forces[:, self.feet_indices, 2] > 5.).float()
        stance_mask = self._get_gait_phase().float()
        
        target_contact = stance_mask[:, 0].unsqueeze(1).repeat(1, 4)
        contact_error = torch.sum(torch.abs(contact - target_contact), dim=1)

        JUMP_reward = torch.exp(-contact_error)

        vel_scale = torch.clip(self.base_lin_vel[:, 0] / 0.5, min=0.0, max=1.0)

        return JUMP_reward * vel_scale
    ...
    def _reward_default_hip_pos(self):
        # 0, 3, 6, 9 是四条腿的 Hip 关节
        hip_indices = [0, 3, 6, 9]
        # 计算当前劈叉角度与“健康站立角度”的绝对误差
        hip_error = torch.sum(torch.abs(self.dof_pos[:, hip_indices] - self.default_dof_pos[:, hip_indices]), dim=1)
    ...
    #     swing_mask = 1 - self._get_gait_phase()
    #     # feet height should larger than target feet height at the peak
    #     rew_pos = torch.clip(self.feet_height,min=0,max=0.25)  #max=0.5
    ...
    def _reward_feet_clearance(self):
        ...
        # Trot 专用的抬脚掩码 
        swing_mask = 1 - self._get_gait_phase()
        rew_pos_trot = torch.clip(self.feet_height, min=0, max=0.08) # 跑不用抬太高
        # 左右对角腿分别乘上各自的 mask
        rew_pos_trot = rew_pos_trot[:, 0] * swing_mask[:, 0] + rew_pos_trot[:, 1] * swing_mask[:, 1] + \
                       rew_pos_trot[:, 2] * swing_mask[:, 1] + rew_pos_trot[:, 3] * swing_mask[:, 0] 

        # Jump 专用的抬脚掩码 (四腿同步)
        rew_pos_jump = torch.clip(self.feet_height, min=0, max=0.15)
        rew_pos_jump = torch.sum(rew_pos_jump * swing_mask[:,:1].repeat(1,4), dim=1)

        # 根据 jump_command 动态切换
        is_jumping = (self.jump_command[:, 0] == 1)
        is_trotting = (self.jump_command[:, 0] == 0)
        
        return rew_pos_trot * is_trotting + rew_pos_jump * is_jumping
    ...
    def _reward_base_height(self):
        ...
        # Jump 模式：只防下蹲，不防升天 (允许高跳)
        error_jump = (self.cfg.rewards.base_height_target - base_height).clip(min=0.)
        
        # Trot 模式：允许上下微小的起伏，不需要太严苛 (把 *10 改成 *2，容错率拉满)
        error_trot = torch.abs(base_height - self.cfg.rewards.base_height_target)
        
        is_jumping = (self.jump_command[:, 0] == 1)
        is_trotting = (self.jump_command[:, 0] == 0)
        
        rew_jump = torch.exp(-error_jump * 10)
        rew_trot = torch.exp(-error_trot * 2) 
        
        return rew_trot * is_trotting + rew_jump * is_jumping

    # def _reward_base_height(self):   #jump
    ...
    def _reward_torque_limits(self):  #jump
    ...
    def _reward_tracking_lin_vel(self):
        ...
        reward=torch.exp(-lin_vel_error/self.cfg.rewards.tracking_sigma)*(torch.norm(self.commands[:, :2], dim=1) > 0.2)  #jump
        reward+=torch.exp(-torch.norm(self.base_lin_vel[:, :2])/self.cfg.rewards.tracking_sigma)*(torch.norm(self.commands[:, :2], dim=1) < 0.2) #jump
    ...
    def _reward_tracking_ang_vel(self):
        ...
        reward =torch.exp(-ang_vel_error/self.cfg.rewards.tracking_sigma)*(torch.abs(self.commands[:, 2]) > 0.2)  #jump
        reward+=torch.exp(-torch.abs(self.base_ang_vel[:, 2])/self.cfg.rewards.tracking_sigma)*(torch.abs(self.commands[:, 2]) < 0.2) #jump
        # return torch.exp(-ang_vel_error/self.cfg.rewards.tracking_sigma)*(self.trot>0.78)
        return reward

    def _reward_feet_air_time(self):   #jump
        # Reward long steps
        ...
        rew_airTime = torch.sum((self.feet_air_time - 0.1).clip(min=0.) * first_contact, dim=1)
        # rew_airTime *= torch.norm(self.commands[:, :2], dim=1) > 0.1 #no reward for zero command
        self.feet_air_time *= ~contact_filt

        return rew_airTime

    def _reward_feet_stumble(self):   #jump
