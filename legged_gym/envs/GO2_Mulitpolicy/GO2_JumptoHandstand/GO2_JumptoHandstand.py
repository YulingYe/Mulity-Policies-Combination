...
class GO2_JumptoHandstand_Robot(BaseTask):
    def __init__(self, cfg: GO2_JumptoHandstand_Cfg, sim_params, physics_engine, sim_device, headless):
    ...
    def step(self, actions):
        ...
        for _ in range(self.cfg.control.decimation):
            action_delayed = self.update_cmd_action_latency_buffer()#????放在这里表示延迟了0.005秒的1到3帧月也就是最多0.015秒 影响hangstand
            #如果放到外面延迟最多能达到0.1秒是不符合常理的
            self.torques = self._compute_torques(action_delayed).view(self.torques.shape) #影响hangstand
            ...
            self.update_obs_latency_buffer() #影响handstand
    ...
    def set_policy_mode(self, stand_mode, p_gain=None, d_gain=None, cycle_time=None, reset_history=True):
        self.stand_command[:] = float(stand_mode)
        if p_gain is not None:
            self.p_gains[:] = p_gain
        if d_gain is not None:
            self.d_gains[:] = d_gain
        if cycle_time is not None:
            self.cfg.rewards.cycle_time = cycle_time
        ...
        if reset_history:
            ...
            for i in range(self.critic_history.maxlen):
                self.critic_history[i][:] = 0.
            self._reset_latency_buffer(env_ids)

            # Rebuild the stacked observations under the new mode so the next policy
            # step does not consume stale jump/handstand history.
            self.compute_observations()
            latest_obs = self.obs_history[-1].clone()
            latest_critic_obs = self.critic_history[-1].clone()
            for i in range(self.obs_history.maxlen):
                self.obs_history[i][:] = latest_obs
            for i in range(self.critic_history.maxlen):
                self.critic_history[i][:] = latest_critic_obs
            self.obs_buf = torch.cat(list(self.obs_history), dim=1)
            self.privileged_obs_buf = torch.cat(list(self.critic_history), dim=1)

    def get_policy_observation(self, stand_mode):
        if stand_mode:
            return self.obs_history[-1].clone()
