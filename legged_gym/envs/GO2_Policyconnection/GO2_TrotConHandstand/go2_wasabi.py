...
# Copyright (c) 2024-2025 Yuling Ye
...
import torch
import sys
sys.path.append("/media/laoye/8AE48189E4817869/UBuntu2004Linux/unitree_rl_gym/rsl_rl/rsl_rl/")

# go2-gym
from legged_gym.envs import LeggedRobot
from legged_gym import LEGGED_GYM_ROOT_DIR
from .go2_wasabi_config import GO2RoughCfg
...
class Go2(LeggedRobot):
    ...
    def post_physics_step(self):
        ...
        self.foot_velocities = self.rigid_body_state.view(self.num_envs, self.num_bodies, 13
                                                          )[:, self.feet_indices, 7:10] #足部速度
        # self.foot_positions = self.rigid_body_state.view(self.num_envs, self.num_bodies, 13)[:, self.feet_indices,
        #                       0:3] #足部位置坐标
        #print("---------------selfs.feet_indices---------",self.feet_indices)
        #print("------------self.foot_positions-----shape-----------",self.foot_positions.shape)
        

        self._post_physics_step_callback() #发布机器人运动状态

        # compute observations, rewards, resets, ...
        self.check_termination() #检查环境是否需要被重置
        self.wasabi_record_states()
        self.compute_reward()
        env_ids = self.reset_buf.nonzero(as_tuple=False).flatten()

        #print("----env_ids--post_physics_step--",env_ids)
    ...
    def compute_observations(self):
        ...
        self.obs_buf = torch.cat(
            (
                #self.base_lin_vel * self.obs_scales.lin_vel,  # 真实环境下不可得
        ...
        # print("self.base_lin_vel size:",self.base_lin_vel.shape)
        # print("self.base_ang_vel size:",self.base_ang_vel.shape)
        # print("self.projected_gravity:",self.projected_gravity.shape)
        # print("self.commands size:",self.commands[:, 0].unsqueeze(1).shape)
        # print("self.dof_pos size:",self.dof_pos.shape)
        # print("self.dof_vel size:",self.dof_vel.shape)
        # print("self.actions size:",self.actions.shape)
    ...
    def compute_reward(self):
        ...
        for i in range(len(self.reward_functions)):
            #print("len-------",len(self.reward_functions))
    ...
    def _init_buffers(self):
        ...
        self.dof_vel_limits[:, self.hip_indices] = 5.0  #14.0
        #print("self.hip_indices",self.hip_indices)
    ...
    def reset_idx(self, env_ids):
        ...
        Args:
            env_ids (list[int]): List of environment ids which must be reset env_ids:需要重置的环境编号
        """
        if len(env_ids) == 0:
            return
        # update curriculum
        ...
        if self.cfg.domain_rand.reference_state_initialization:
            frames = self.motion_loader.get_frames(len(env_ids)) #根据需要复位的环境序号，读取参考文件信息
    ...
    def _reset_dofs(self, env_ids, frames, env_ids_mask):
        if frames is not None:
            ...
            #print("frames[env_ids_mask]----shape--",frames[env_ids_mask].shape)
    ...
    def _reset_root_states(self, env_ids, frames, env_ids_mask):
        if frames is not None:
            #print("env_ids_mask---------",env_ids_mask)
