...
class MotionLoader:
    ...
    def __init__(self, device, motion_file=None, corruption_level=0.0, reference_observation_horizon=2, test_mode=False, test_observation_dim=None):
        ...
        self.reference_state_idx_dict_file = LEGGED_GYM_ROOT_DIR + "/resources/robots/go2/datasets/reference_state_idx_dict.json"
        ...
        self.data = self._data_corruption(loaded_data, level=corruption_level) #数据修正
        self.num_motion_clips, self.num_steps, self.reference_full_dim = self.data.size() #运动参考加载
        ...
        # Preload transitions
        self.num_preload_transitions = 50000  #50000
        motion_clip_sample_ids = torch.randint(0, self.num_motion_clips, (self.num_preload_transitions,), device=self.device)
        print("motion_clip_sample_ids--------------------",motion_clip_sample_ids)
        print("motion_clip_sample_ids---------shape-----------",motion_clip_sample_ids.shape)
        ...
        for i in range(self.reference_observation_horizon):
            self.preloaded_states[:, i] = self._get_frame_at_step(motion_clip_sample_ids, step_sample + i) #预加载模型，总reference_observation_horizon为2
            print("self.preloaded_states[:, i]---------",self.preloaded_states[:, i].shape)
        if test_mode:
    ...
    def get_frames(self, num_frames):
        #随机选取运动参考frame
        ids = torch.randint(0, self.num_preload_transitions, (num_frames,), device=self.device) #随机张量生成
        #print("num_frames------",num_frames)
        #print("self.preloaded_states-----self.preloaded_states[ids, 0]--shape------",self.preloaded_states[ids, 0].shape)
    ...
    def quaternion_slerp(self, quat_low, quat_high, blend):
        ...
        angle = 2 * torch.acos(relative_quat[:, -1]).unsqueeze(-1) #归一化以后求相对旋转的反余弦值
    ...
    def get_base_quat(self, frames):
        if "base_quat" in self.state_idx_dict:
            ...
            raise Exception("[] base_quat not specified in the state_idx_dict")
